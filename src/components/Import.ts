...
function renderImportForm(): m.Vnode {
  return m('div', [
    m('.card', [
...
function renderSummary(): m.Vnode {
  return m('.card', [
    m('.stats', [
...
function renderPreview(): m.Vnode | null {
  if (!S.data || S.step !== 'import') return null
  return m('.card.section-gap', [
